#!/usr/bin/env python3
"""
Walk-forward validation of RSI+Bollinger-Bands-only 1h, isolated from the
3-strategy blend. RSI_BB has only ever been tested bundled inside
ema_crossover+rsi_bb+macd_volume runs, so it's never been possible to tell
whether it has any edge on its own.

Runs the same 6 non-overlapping 30-day windows as walk_forward_ema_only.py
and walk_forward_macd_only.py for a direct comparison.
"""
import sys
...
def main():
    ...
    for i in range(NUM_WINDOWS):
        ...
        result = run_backtest(
            days_back=DAYS_BACK,
            config_overrides=dict(RSI_BB_ONLY_OVERRIDES),
            save_report=True,
            end_date=end_date,
        )
        summary.append({
