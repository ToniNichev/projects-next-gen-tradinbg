#!/usr/bin/env python3
"""
Add 5 random test trades to the database for chart testing.
Trades are spread over the last 10 days.
"""
...
def generate_test_trades():
    """Generate 5 random trades spread over 10 days"""
    ...
    # Generate 5 trades spread over 10 days
    trades = []

    for i in range(5):
        # Spread trades over 10 days (randomly distributed)
        days_back = random.uniform(0, 10)
        timestamp = now - timedelta(days=days_back)

        # Random price variation (±$5000)
        ...
        signal_price = price
        short_ema = price + random.uniform(-1000, 1000)
        long_ema = price + random.uniform(-2000, 2000)
        trend_strength = random.uniform(0.1, 1.0)
        rsi = random.uniform(20, 80)
        atr = random.uniform(500, 2000)
        position_size = random.uniform(0.1, 0.5)  # 10-50% position size
        stop_loss = price * (0.98 if side == "buy" else 1.02)  # 2% stop loss
        take_profit = price * (1.04 if side == "buy" else 0.96)  # 4% take profit
    ...
    print(f"Adding {len(trades)} test trades to database...")
    ...
    for i, (timestamp, trade_data) in enumerate(trades):
        try:
            ...
            print(f"✅ Added {side.upper()} trade #{i+1}: {amount:.6f} BTC @ ${price:.2f} on {timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
        except Exception as e:
            print(f"❌ Failed to add trade #{i+1}: {e}")

    print("\nTest trades added successfully!")
    print("You can now view them in the dashboard chart at /ui")
